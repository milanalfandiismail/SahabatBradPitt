/**
 * admin/actors.js
 * Handles: Sineas (Actors) CRUD, List, Editor Forms, Image Preview.
 * Target < 250 lines.
 */

window.actorsCurrentPage = 1;
window.actorsSearchQuery = "";
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (results.length === 0) {
                ...
                if (actorsPaginationInfo) actorsPaginationInfo.textContent = "Menampilkan 0 sineas";
                if (actorsPaginationControls) actorsPaginationControls.textContent = "";
                return;
            }
        ...
        .catch(() => {
            actorsLoading?.classList.add('hidden');
            showToast('Gagal memuat data sineas.', 'error');
        });
}
...
function openActorEditor(actor) {
    ...
    if (actor) {
        window.selectedActorId = actor.id;
        ...
        if (preview && placeholder) {
            ...
            if (actor.local_photo) {
                ...
                photoUrl = actor.tmdb_photo.startsWith('http') ? actor.tmdb_photo : `https://image.tmdb.org/t/p/w400${actor.tmdb_photo}`;
            }
            if (photoUrl) {
...
function deleteActor(id, name) {
    const toast = _buildConfirmToast(`Hapus Sineas '${name}' dari database secara permanen?`, () => {
        secureFetch(`/api/actors/${id}/`, { method: 'DELETE' })
            .then(res => { if (!res.ok) throw new Error(); showToast('Sineas terhapus.', 'success'); fetchActors(window.actorsCurrentPage); })
            .catch(() => showToast('Gagal menghapus sineas.', 'error'));
    });
    document.body.appendChild(toast);
}

// Binds all actor search & uploader elements when DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('actor-form')?.addEventListener('submit', e => {
        ...
        secureFetch(url, { method, body: formData })
            ...
            .then(() => { showToast('Profil Sineas berhasil disimpan!', 'success'); closeActorEditor(); fetchActors(window.actorsCurrentPage); })
            .catch(() => showToast('Gagal menyimpan profil Sineas.', 'error'));
    });
});

// Expose functions to window context
window.fetchAllActorsForCast = fetchAllActorsForCast;
window.fetchActors = fetchActors;
window.renderActorsTable = renderActorsTable;
window.renderActorsPagination = renderActorsPagination;
window.openActorEditor = openActorEditor;
window.closeActorEditor = closeActorEditor;
